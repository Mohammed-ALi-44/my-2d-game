...
public class NinjaPlayerController : MonoBehaviour {
	...
	void SwordAttack()
    {
		if (Time.time > nextFire)
		{
			...
			if (isFacingRight)
			{
				Instantiate(swordRange, bowTip.position, Quaternion.Euler(new Vector3(0, 0, 0)));
			}
			else if (!isFacingRight)
			{
				Instantiate(swordRange, bowTip.position, Quaternion.Euler(new Vector3(0, 0, 180f)));
