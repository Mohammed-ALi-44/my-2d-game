...
public class SantaPlayerController : MonoBehaviour
{
	...
	void SnowArrow()
	{
		if (Time.time > nextFire)
		{
			...
			{	
				...
				Instantiate(arrow2, bowTip.position, Quaternion.Euler(new Vector3(0, 0, 0)));
			}
			else if (!isFacingRight)
			{
				Instantiate(arrow2, bowTip.position, Quaternion.Euler(new Vector3(0, 0, 180f)));
